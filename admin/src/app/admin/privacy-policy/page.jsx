...
export default function PrivacyPolicyAdminPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setActiveField(null);
      setIsEditMode(false);
      showToast("✅ Privacy Policy পেজ আপডেট হয়েছে", "success");
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setOriginalData(structuredClone(keyed));
      setActiveField(null);
      showToast("🔄 Privacy Policy পেজ রিসেট হয়েছে", "success");
    } catch (err) {
      showToast(`❌ ${err.message}`, "error");
    } finally {
      setResetting(false);
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-5 pb-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-slate-100">🔒 Privacy Policy Page</h2>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl p-5 max-w-sm w-full">
            <p className="font-bold text-gray-800 dark:text-slate-200 mb-2">
              Privacy Policy পেজ রিসেট করবেন?
            </p>
