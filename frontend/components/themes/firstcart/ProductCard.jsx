...
// FirstCart: soft modern card — 4:5 image with rounded-2xl corners, small
// floating heart (top-right corner), full-width "Add to cart"
    ...
    <div ref={cardRef} className="group flex flex-col">
      ...
      <div className="relative aspect-[4/5] w-full overflow-hidden rounded-2xl bg-slate-100">
        ...
        <button
          ...
          className={`absolute right-1 top-1 z-10 flex h-6 w-6 items-center justify-center rounded-full shadow-sm backdrop-blur transition active:scale-90 ${
            isInWishlist
      ...
      <div className="flex flex-1 flex-col gap-1.5 px-1 pb-1 pt-3">
        ...
        {!isOutOfStock && (
          <p className="flex items-center gap-1.5 text-[11px] font-medium text-[var(--theme-text)]/55">
        ...
        {/* Mobile / touch: full-width stacked buttons (side-by-side wraps on narrow 2-col cards) */}
        <div className="mt-auto flex flex-col gap-1.5 pt-1 md:hidden">
          <button
            type="button"
            onClick={onAddToCart}
            disabled={isOutOfStock || inCart}
            className={`flex h-9 w-full items-center justify-center whitespace-nowrap rounded-full text-xs font-semibold transition ${addBtnState}`}
          >
            {addLabel}
          </button>
          <button
            ...
            onClick={handleBuyNow}
            disabled={isOutOfStock}
            className={`flex h-9 w-full items-center justify-center whitespace-nowrap rounded-full border text-xs font-semibold transition ${
              isOutOfStock
