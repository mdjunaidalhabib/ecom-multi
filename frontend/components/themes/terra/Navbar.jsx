...
// Shop Start: clean white navbar (and white mobile bottom tab bar) with a
// hairline rule, monogram brand mark, understated uppercase links and
// theme-primary accents for active states. Reuses the same functional
...
export default function TerraNavbar() {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 border-b border-[var(--theme-text)]/10 bg-white text-[var(--theme-text)] shadow-sm">
        <div className="mx-auto flex h-16 w-full max-w-[1280px] items-center justify-between gap-4 px-4 sm:px-6">
          <button
            onClick={() => setMenuOpen(true)}
            className="-ml-1 p-1 text-[var(--theme-text)] transition-colors hover:text-[var(--theme-primary)] md:hidden"
          ...
          <Link href={base || "/"} className="flex min-w-0 flex-1 items-center gap-2 md:flex-none md:gap-3">
            {navbar?.brand?.logo && !imgError ? (
              <img
                ...
                alt={brandName || "Brand"}
                className="h-8 w-8 rounded-lg object-cover ring-2 ring-[var(--theme-primary)]/20 md:h-10 md:w-10"
                onError={() => setImgError(true)}
              />
            ) : (
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[var(--theme-primary)] text-sm font-bold md:h-10 md:w-10 md:text-base text-white shadow-sm shadow-[var(--theme-primary)]/30">
                {(brandName.charAt(0) || "T").toUpperCase()}
              </div>
            )}
            <span className="truncate text-sm font-semibold uppercase tracking-[0.12em] text-[var(--theme-text)] md:text-lg md:tracking-[0.14em]">
              {brandName}
            </span>
          </Link>

          {/* Desktop search — sits before the menu links, themed to match */}
          <div className="hidden min-w-0 md:block [&_input]:w-52 [&_input]:rounded-full [&_input]:border-[var(--theme-text)]/15 [&_input]:bg-[var(--theme-text)]/[0.04] [&_input]:px-4 [&_input]:py-1.5 [&_input]:text-sm [&_input]:font-medium [&_input]:text-[var(--theme-text)] [&_input]:placeholder:font-normal [&_input]:placeholder:text-[var(--theme-text)]/45 [&_input:focus]:border-[var(--theme-primary)] [&_input:focus]:bg-white [&_input:focus]:ring-2 [&_input:focus]:ring-[var(--theme-primary)]/20 lg:[&_input]:w-60 [&_button.rounded-xl]:bg-[var(--theme-primary)]/5 [&_button.rounded-xl:hover]:bg-[var(--theme-primary)]/15">
          ...
          <div className="hidden items-center gap-7 md:flex">
            {NAV_LINKS.map(({ href, label }) => (
              ...
              >
                {label}
                <span
                  className={`absolute -bottom-1 left-0 h-0.5 rounded bg-[var(--theme-primary)] transition-all duration-300 ${
          ...
          <div className="flex items-center gap-1 text-[var(--theme-text)]">
            <button
              className="p-2 text-[var(--theme-text)] transition-colors hover:text-[var(--theme-primary)] md:hidden"
              onClick={() => setMobileSearchOpen(true)}
              aria-label="Search"
            >
              <FaSearch className="h-4 w-4" />
            </button>
            <div className="mr-1 hidden md:block [&>button]:rounded-full [&>button]:border [&>button]:border-[var(--theme-primary)] [&>button]:bg-transparent [&>button]:px-4 [&>button]:py-1.5 [&>button]:text-[12px] [&>button]:font-bold [&>button]:uppercase [&>button]:tracking-wider [&>button]:text-[var(--theme-primary)] [&>button:hover]:bg-[var(--theme-primary)] [&>button:hover]:text-white [&>div>button:first-child]:rounded-full [&>div>button:first-child]:border [&>div>button:first-child]:border-[var(--theme-text)]/15 [&>div>button:first-child]:bg-transparent [&>div>button:first-child]:py-1 [&>div>button:first-child]:pl-1 [&>div>button:first-child]:pr-4 [&>div>button:first-child]:text-[12px] [&>div>button:first-child]:font-bold [&>div>button:first-child]:uppercase [&>div>button:first-child]:tracking-wider [&>div>button:first-child]:text-[var(--theme-text)] [&>div>button:first-child:hover]:border-[var(--theme-primary)] [&>div>button:first-child:hover]:bg-[var(--theme-primary)]/10 [&>div>button:first-child:hover]:text-[var(--theme-primary)] [&_a:hover]:bg-[var(--theme-primary)]/10 [&_a:hover]:text-[var(--theme-primary)]">
              <AccountMenuDesktop />
            </div>
            <div className="hidden p-2 transition-colors hover:text-[var(--theme-primary)] md:block [&_span.bg-red-500]:bg-[var(--theme-primary)] [&_span.bg-red-500]:font-bold [&_span.bg-red-500]:text-white">
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-[var(--theme-text)]/10 bg-white shadow-[0_-4px_16px_rgba(0,0,0,0.06)] md:hidden">
        <div className="grid grid-cols-5 items-start text-[var(--theme-text)]/60 leading-4">
          {[
            { key: "home", path: "/", node: (
            ...
            { key: "wishlist", path: "/wishlist", node: (
              <div className="text-[10.5px] font-medium [&_svg]:h-5 [&_svg]:w-5 [&_a]:gap-0.5 [&_span.bg-red-500]:bg-[var(--theme-primary)] [&_span.bg-red-500]:font-bold [&_span.bg-red-500]:text-white">
              ...
              >
                {me?.avatar ? (
                  <Image
                    ...
                    width={20}
                    height={20}
                    className="h-5 w-5 rounded-full object-cover ring-1 ring-[var(--theme-primary)]/40"
                  />
                ) : (
            ...
            <div
              key={key}
              className={`relative flex min-w-0 justify-center px-0.5 pb-2 pt-2.5 transition-colors ${isActive(path) || (also && isActive(also)) ? "font-bold text-[var(--theme-primary)]" : ""}`}
            >
              <span
                className={`absolute inset-x-6 top-0 h-[3px] rounded-b bg-[var(--theme-primary)] transition-opacity ${
